...
int receive_REQUEST_SET_PRICE(int socket,auction_message *msg)
{
	...
	int a= recv_a_int(socket,&(param->price));if(a!=0){return -1;}
	(*msg).data=param;
...
int receive_RESPOND_SET_PRICE(int socket,auction_message *msg){
	...
	param.message[0]='\0';
	int a= recv_a_string(socket,(param.message));if(a!=0){return -1;}
	msg->data=&param;
	return check_error_auction(a,0,0);
}
int receive_NOTIFY_NEW_PRICE(int socket,auction_message *msg){
	...
	int a= recv_a_int(socket,&(param.newprice));if(a!=0){return -1;}
	int b= recv_a_string(socket,(param.winner_name));if(b!=0){return -1;}
	int c=0;
	int d= recv_a_int(socket,&(param.count));if(d!=0){return -1;}
	msg->data=&param;
...
int receive_AUCTION_REQUEST_BUY_NOW(int socket,auction_message *msg){
	...
	int a= recv_a_int(socket,&(param.item_id));if(a!=0){return -1;}
	int b= recv_a_int(socket,&(param.price));if(b!=0){return -1;}
	msg->data=&param;
	return check_error_auction(a,b,0);
}
int receive_AUCTION_RESPOND_BUY_NOW(int socket,auction_message *msg){
	...
	param.message[0]='\0';
	int a= recv_a_int(socket,&(param.item_id));if(a!=0){return -1;}
	int b= recv_a_string(socket,(param.message));if(b!=0){return -1;}
	msg->data=&param;
	return check_error_auction(a,b,0);
}
...
int send_REQUEST_SET_PRICE(int socket,SET_PRICE_PARAM param){
	int a= send_a_int(socket, REQUEST_SET_PRICE);if(a!=0){return -1;}
	int b=send_a_int(socket, param.price);if(b!=0){return -1;}
	return check_error_auction(a, b, 0);
}
int send_RESPOND_SET_PRICE(int socket,SET_PRICE_RESPOND data){
	int a= send_a_int(socket, RESPOND_SET_PRICE);if(a!=0){return -1;}
...
int send_NOTIFY_NEW_PRICE(int socket,NOTIFY_NEW_PRICE_RESPOND data){
	int a= send_a_int(socket, NOTIFY_NEW_PRICE);if(a!=0){return -1;}
	int b= send_a_int(socket, data.newprice);if(b!=0){return -1;}
	int c=send_a_string(socket, data.winner_name);if(c!=0){return -1;}
...
int send_AUCTION_REQUEST_BUY_NOW(int socket,AUCTION_BUY_NOW_PARAM data){
	int a= send_a_int(socket, AUCTION_REQUEST_BUY_NOW);if(a!=0){return -1;}
	int b= send_a_int(socket,data.item_id);if(b!=0){return -1;}
	int c= send_a_int(socket,data.price);if(c!=0){return -1;}
	int ans = check_error_auction(a, b, 0);
	return ans;
}
int send_AUCTION_RESPOND_BUY_NOW(int socket,AUCTION_BUY_NOW_RESPOND data){
	int a= send_a_int(socket, RESPOND_EXIT_ROOM);if(a!=0){return -1;}
	int b= send_a_int(socket,data.item_id);if(b!=0){return -1;}
	int c= send_a_string(socket,data.message);if(c!=0){return -1;}
	int ans = check_error_auction(a, b, 0);
	return ans;
}
